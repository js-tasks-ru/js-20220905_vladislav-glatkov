export default class DoubleSlider {
  ...
  initEventListeners() {
    this.subElements.sliderLeft.addEventListener("pointerdown", (event) =>
      this.onSliderDown(event)
    );
    this.subElements.sliderRight.addEventListener("pointerdown", (event) =>
      this.onSliderDown(event)
    );
  }
  ...
  onSliderMove = (event) => {
    ...
    if (this.dynamicSlider === this.subElements.sliderRight) {
      ...
      this.dynamicSlider.style.right = this.subElements.progress.style.right =
        currentRight + "%";
    ...
    if (this.dynamicSlider === this.subElements.sliderLeft) {
      ...
      this.dynamicSlider.style.left = this.subElements.progress.style.left =
        currentLeft + "%";
  ...
  calcDiff() {
    ...
    const from = Math.round(
      this.min + parseFloat(left) * (this.max - this.min) * 0.01
    );
    const to = Math.round(
      this.max - parseFloat(right) * (this.max - this.min) * 0.01
    );
